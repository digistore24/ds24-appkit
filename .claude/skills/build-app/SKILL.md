...
- **No idea, or a vague one** ("don't know", "something with…", an industry,
  "roughly, but it's fuzzy") → start **`market-research`** NOW and ask nothing
  yourself first — not the idea, not expertise, not reach: it asks all of that,
  in its order. 🚨 Their word decides, not yours: an idea that sounds buildable
  once you hear it is still THIS branch — measured, one run each way.
