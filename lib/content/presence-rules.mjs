...
export function presenceProblems(reports) {
  ...
  for (const report of reports) {
    ...
    // A report with neither `items` nor `unanswered` is an app older than this
    // checkout — the case this file's header is written for. Reading it as a
    // TypeError loses the sentence prepared three lines up.
