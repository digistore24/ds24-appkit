 ...
 */
export async function sendS3(settings, method, key, body, contentType, extraHeaders, signal) {
  ...
  return fetch(`${settings.endpoint}${path}`, {
    ...
    redirect: "manual",
    // 🚨 Optional, and the default is exactly today's behaviour — but without it
    // a caller that BOUNDS its wait cannot: a bucket that accepts the connection
    // and never answers holds the request open for as long as the process runs.
    // `lib/ops/health.ts` built an `AbortSignal.timeout()` for its media probe
    // and had nowhere to hand it, so its `timedOut` code was unreachable and the
    // watchdog's job could hang holding the cron lock.
    signal,
  });
}
