...
/**
 * The currency an operator is, by a large majority, billed in — per language.
 *
 ...
 *
 * `en` is deliberately USD: it is the language the rest of the world shares,
 * and its most likely operator is not in the euro area.
 *
 * 🚨 One entry per language in `LOCALES` (`i18n/config.ts`), and none besides
 * — `pricing.test.ts` holds both directions. This used to be a Set of the euro
 * languages, and a language missing from a Set is indistinguishable from one
 * decided as USD: a customer swapped Spanish for Portuguese along the eight
 * steps `docs/locales.md` names, and this table was not among them, so a
 * Portuguese operator would have been sent to USD by silence (2026-09-08). A
 * decision has to be WRITTEN so that its absence is a test failure.
 */
export const CURRENCY_BY_LOCALE = Object.freeze({
  de: "EUR",
  en: "USD",
  es: "EUR",
  fr: "EUR",
