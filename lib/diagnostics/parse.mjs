 ...
 *   inelegance.
 *
 * 🚨 **More than one bracket, and a colon after them.** Auth.js prints
 * `[auth][error] EmailSignInError: Connection timeout` and then
 * `[auth][cause]: Error: …` — two groups with no space between, and a colon
 * after the last. With one group allowed, a sign-in whose mail could not leave
 * was invisible here: measured 2026-09-16 on the exact line a tester's
 * deployed app logged on every sign-in, 0 findings.
 */
...
const HINTS = [
  ...
  },
  {
    ...
    say: "the key is missing in one messages/<code>.json — every language needs it (LOCALES in i18n/config.ts).",
  },
  {
    when: /MISSING_FORMAT/,
  ...
  {
    // BEFORE the generic connection hint below: `HINTS.find` takes the first
    // match, and "is the database up?" is the wrong direction for a mail that
    // cannot leave. `Connection timeout` and `Greeting never received` are
    // nodemailer's own words; `[mail]` is the boot probe's line
    // (lib/diagnostics/smtp-probe.mjs).
    when: /\[mail\]|Connection timeout|Greeting never received|\bSMTP\b.*\b(?:ETIMEDOUT|ECONNREFUSED|ESOCKET)\b/,
    say:
      "the app could not open a connection to its mail server. On a host that is\n" +
      "    rarely the credentials: many hosts block outbound SMTP (Railway below its\n" +
      "    Pro plan). Switch to an HTTPS transport — node run.mjs mail-setup → Brevo or\n" +
      "    Postmark, the new variables at the host, the SMTP ones removed, redeploy.\n" +
      "    node run.mjs health --url <address> says whether the host reaches its mail server.",
  },
  {
...
export function parseErrors(text) {
  // Colour codes out first. A library that colours its own prefix (Auth.js
  // writes `\x1b[31m[auth][error]\x1b[0m`) does so on a host too, where
  // nothing is a terminal, and every anchored pattern below then starts on an
  // escape byte instead of a bracket. `/\r?\n/` for the same reason every
  // splitter here uses it.
