...
//
// 🚨 Read, never typed out. `basic_monthly` stood here, and `CLAUDE.md` tells
// every app to replace the example products — so this file went red in every
// real app, for a defect that was not there. `planShapedKey()` picks whatever
// this registry holds; `keysOrSkip()` inside each test says `⏭ NOT CHECKED`
// with the reason when it holds nothing of that shape, because no key can stand
// in for one (`lib/digistore/test-product-keys.ts`).
...
describe("onPaymentEvent — which transition reaches the entitlement", () => {
  it("a paid subscription ACTIVATES, for the identified member and that product", async (ctx) => {
  ...
  it("🚨 a redelivered payment does not revive a grant that a refund ended", async (ctx) => {
    const [plan] = keysOrSkip(ctx, PLAN_PICK);
  ...
  it("a payment on a SUSPENDED grant resumes it rather than activating a second", async (ctx) => {
    const [plan] = keysOrSkip(ctx, PLAN_PICK);
    purchaseGrant.mockResolvedValue({
      memberId: MEMBER,
      productKey: plan,
...
describe("onPaymentEvent — whose grant it acts on", () => {
  it("falls back to the grant's own owner when the identity no longer resolves", async (ctx) => {
    const [plan] = keysOrSkip(ctx, PLAN_PICK);
...
describe("onPaymentEvent — a token package credits a balance, not a grant", () => {
  // Whatever token package this app sells, and the credits IT declares — both
  // read off `config/digistore-products.json` rather than named here. The two
  // used to be `"starter"` and `1000`, which is one shipped example and one
  // number out of it: an app that renamed the package OR changed its size took
  // this block down with it.
  const TOKEN_PICK = tokenKey();
  const CREDITS = TOKEN_PICK.key ? getTokenPackage(TOKEN_PICK.key).credits : 0;

  function tokenPayload(event: string, over: Record<string, string> = {}) {
    return payload(event, {
      custom: `m:${MEMBER};t:${TOKEN};p:${TOKEN_PICK.key ?? ""}`,
      ...over,
    });
  }
