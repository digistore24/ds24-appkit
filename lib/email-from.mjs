...
//
// The same holds for WHICH transport is configured (`mailTransport()` below):
// the sender depends on it, the boot guard requires one, the send path
// dispatches on it, and the wizard's test mail has to take the same road. It
// used to be decided in three places — a private boolean here, the `has*Config`
// trio in env-guard, and a copy inside mail-setup — which was fine for two
// transports and is exactly how a third one gets forgotten in one of them.
...
/**
 * Which mail transport this environment sends through — the ONE decision, read
 * by the boot guard, the send path (`lib/mail-send.mjs`), the operator notices
 * and the wizard's test mail.
 *
 * Each needs its full pair (or triple): a token without a sender is not a
 * transport, it is half a setup. When more than one is complete the order is
 * Postmark, then Brevo, then SMTP — the order `deliver()` has always had, so an
 * existing app that adds a second set of variables keeps sending the way it did.
 * `mail-setup` comments the other two out, so that case is a hand edit.
 *
 * Brevo and Postmark are HTTPS APIs; SMTP is a raw connection to port 587/465,
 * which several hosts block outright on their cheaper plans — the reason there
 * is more than one way here at all (docs/auth-setup.md).
 *
 * @param {Record<string, string | undefined>} env
 * @returns {"postmark" | "brevo" | "smtp" | "none"}
 */
export function mailTransport(env) {
  if (env.POSTMARK_SERVER_TOKEN && env.POSTMARK_SENDER) return "postmark";
  if (env.BREVO_API_KEY && env.BREVO_SENDER) return "brevo";
  ...
  const own =
    transport === "postmark"
      ...
      : transport === "brevo"
        ? env.BREVO_SENDER
        : env.SMTP_FROM;
  const from = own || env.EMAIL_FROM || "";
  return from.trim() || null;
}

/**
 * An address split into the two parts an HTTP mail API wants separately:
 * `"Fangfertig <login@fangfertig.de>"` → `{ email: "login@fangfertig.de",
 * name: "Fangfertig" }`. A bare address has no name. Quotes around the name are
 * dropped — they belong to the header syntax, not to the name.
 *
...
export function splitAddress(address) {
  ...
  if (!angled) return { email: text, name: null };
